...
contract('VestingCtrl', function(accounts) {
    ...
    it("vesting should release gradually", async function() {
        ...
        await token.approve(vesting_ctl.address, ALPHA_UNIT.mul(100*1000000), {from: owner});
        console.log("CREATE VESTING");
        await vesting_ctl.newVesting(investor1, nowTime, 10, 20, ALPHA_UNIT.mul(100*1000000), {from: manager});
        const t5 = sleep(5000);
        ...
        var balance0 = 0;
        console.log("AMOUNT", (await vesting_ctl.totalAmount.call({from: investor1})).div(ALPHA_UNIT).toString());
        await t5;
        await vesting_ctl.release({from: investor1});
        balance0 = await getALPHA(investor1);
