...
contract('Buyback', function(accounts) {
    ...
    it("buyback overflow", async function() {
        ...
        await token.transfer(investor1, ALPHA_UNIT.mul(10000000), {from: owner});
        await token.transfer(investor2, ALPHA_UNIT.mul(10000000), {from: owner});
        console.log("balance: investor1", (await getALPHA(investor1)).toString());
        console.log("balance: investor2", (await getALPHA(investor2)).toString());
        console.log(web3.fromWei(await buyback.convertBalance.call(ALPHA_UNIT.mul(5000000)), "ether").toString());
        await token.approve(buyback.address, ALPHA_UNIT.mul(5000000), {from: investor1});
        await token.approve(buyback.address, ALPHA_UNIT.mul(10000000), {from: investor2});
        ...
        console.log("balance: investor1", (await getALPHA(investor1)).toString());
        console.log("balance: investor2", (await getALPHA(investor2)).toString());
        assert.equal(await getALPHA(investor1), 5000000, "complete buyback for investor1");
        assert.equal(await getALPHA(investor2), 4000000, "partial buyback for investor2");
